...
def read_corpus_yield(path, max_chars=None, max_sent_len=20):
  ...
  :return:
  """
  perm = list(range(1,13))###13번째 파일 다른 형식 이라 빼버렸음. 여긴 새로 딕트 만들었음.
  random.shuffle(perm)
...
def read_korean_conll_corpus(path, max_sent_len=None):
  """
...
def create_batches(x, batch_size, word2id, char2id, config, perm=None, shuffle=True, sort=True, use_cuda=True):
  """
...
class Model(nn.Module):
  ...
  def load_model(self, path):
    ...
    self.token_embedder.load_state_dict(torch.load(os.path.join(path, 'token_embedder.pkl'),map_location="cuda:0"))
    self.encoder.load_state_dict(torch.load(os.path.join(path, 'encoder.pkl'),map_location="cuda:0"))
    self.classify_layer.load_state_dict(torch.load(os.path.join(path, 'classifier.pkl'),map_location="cuda:0"))
...
def train_model(epoch, opt, model, optimizer,
  ...
  for w, c, lens, masks in zip(train_w, train_c, train_lens, train_masks):
    ...
    if cnt % opt.eval_steps == 0 or cnt % 100==0: #len(train_w) == 0:
      if valid is None:
        ...
        if train_ppl < best_train:
          ...
          torch.save({
            ...
            'loss': best_train
          }, os.path.join(opt.model, 'checkpoint.tar'))
      else:
...
def train():
  ...
  if token_embedder_name == 'cnn':
    train_data = read_korean_conll_corpus(opt.train_path, opt.max_sent_len)
  ...
  if opt.cont == 1222:
    checkpoint = torch.load(os.path.join(opt.model, 'checkpoint.tar'))
    optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    print("-------------------------------------------------------")
    print("이전 epoch :",checkpoint['epoch'])
    print("이전 ppl ",checkpoint['loss'])
    print("현재 러닝레이트 : ",optimizer.param_groups[0]['lr'])
    print("-------------------------------------------------------")
